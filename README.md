# Platform Architecture Overview & Documentation
...
docker-compose up -d
```
This runs the CMS backend along with PostgreSQL (metadata DB), Redis (cache), Jaeger (tracing), and Prometheus (metrics).

## Swagger Documentation

Swagger documentation is automatically generated.
To generate/update the Swagger docs, run:
```bash
make swagger
```
