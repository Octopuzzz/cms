...
The `Dockerfile` builds the Go application, and `docker-compose.yml` orchestrates the core platform components along with PostgreSQL and Prometheus.

## Swagger Documentation

Swagger documentation is integrated. If running locally, you can generate the documentation using:
```bash
swag init -g cmd/server/main.go --parseDependency --parseInternal --output docs
```
The generated `docs` directory should not be committed to version control.

## Unit Tests

The system includes comprehensive unit testing with a required minimum coverage of 80% across the Repository, Service, and API handlers.
...
│   ├── graphql/         # GraphQL schema generation and handlers
│   ├── handlers/        # HTTP handlers (REST controllers)
...
├── pkg/
│   ├── logger/          # Structured logger wrapper (Zap/Logrus)
