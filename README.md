...
The project uses a clean, modular structure following Go best practices:

- `cmd/server`: Application entrypoint.
- `internal/api`: API handlers/controllers.
- `internal/services`: Application business logic, covering CMS control plane, CRUD engine, service builder, schema migrations, and backups.
- `internal/database`: Database connectors and schema configurations.
- `internal/graphql`: GraphQL gateway generator using `gqlgen`.
- `internal/tracing`: Observability integration with OpenTelemetry.
- `pkg/logger`: Shared Zap logging utilities.
- `pkg/response`: Standardized pagination, errors, and validation responses.
- `tests/`: Unit and integration test suites.
- `docker/`: Contains supplementary scripts or Docker configurations.

## Unit Testing

The system must maintain robust test coverage, specifically targeting Repository, Service, and API handler layers. Minimum expected coverage is **80%**.

Run unit tests across the project using:
```bash
...
The platform is fully containerized and easy to run locally or in production.

1. Ensure Docker and Docker Compose are installed.
2. The `Dockerfile` handles the Go build process.
3. The `docker-compose.yml` configures the backend service, metadata database (PostgreSQL), Redis (cache), and observability tools (Jaeger, Prometheus).
...
API documentation is generated automatically using `swag` from Go comments. The generated docs reside in the `docs/` directory.
